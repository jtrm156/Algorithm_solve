...
//[[필터링 함수]]
        ...
        arrayOf<Collection<Int>>(arrayListOf(1,2,3), mutableListOf(2,3), setOf(1,3)).filterIsInstance<List<Int>>() // [[1,2,3], [2,3]]
...
//[[형태 변환형 함수]]
        ...
        arrayOf(listOf(1,2,3), listOf(4,5,6), listOf(7,8)).flatMap { it-> it } // [1,2,3,4,5,6,7,8]
        arrayOf(listOf(1,2,3), listOf(4,5,6), listOf(7,8)).flatMap { it: List<Int>-> it.take(2) } // [1,2,4,5,7,8]
        // Collection 내부에 Collection이 들어있을 경우, 내부의 Collection을 펼쳐준다.
        // Collection 을 변환하는 조건에 맞는 값들을 펼쳐준다. /List<선택한 변환> 으로 반환

        arrayOf(listOf(1,2,3), listOf('a', 'b', 'c'), listOf(7,8)).flatMap { it -> it.map{ it.toString() }} // [1,2,3,'a','b','c',7,8]

        array.zip(arrayStr) // 두 조합을 합친다. 앞순서가 Pair 의 first가 된다. /List<Pair<T,R>> 를 반환
        listOf<String>("a", "b", "c", "d").zip(arrayOf(1,2,3)) // [(a,1), (b,2), (c,3)] 만일 두 길이가 차이가 날 경우 작은 것의 size 로 생성된다.
        ...
        val associateByResult1 = customerList.associateBy { it.id } //안의 값을 기준으로 Iterable<T> -> K의 연산에 대해 Map<K, T>를 반환 /Map<R, Customer> 형태를 반환
        val associateByResult2 = customerList.associateBy(Customer::id, Customer::name) // /Map<id, name> 의 형태를 반환
